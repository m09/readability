...
We then introduce scores, so that it becomes possible to distinguish between
good and bad lexical enhancements with a precise metric.

The first one, named $S^{Occ}$, is a log of the basic count of
occurrences of a precise lexical enhancement in our dictionary. We
define it in precise terms as:

\begin{align*}
  S_{w → r}^{Occ} & = \log\frac{\card{w \maps{D} r}}{\card{\_ \maps{D} \_}} \\
  & = \log(\card{w \maps{D} r}) - \log(\card{\_ \maps{D} \_})
\end{align*}

This score is still too basic for our needs. Its main shortcoming is
that it outlines mainly punctuation rewritings. But punctuation
rewriting is too linked to syntax to be performed so bluntly, without
taking any context into account are without considering any parse
information. The following scores try to fix this punctuation focus to
turn it into a lexical focus.

To accomplish this shift, we use a language model. In our experiments
it was trained at the bi-gram level although we would have trained it
at a tri-gram level if it wasn't for a lack of memory on the machine
used to enhance the overall robustness of the scores.
