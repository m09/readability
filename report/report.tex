...
It was also very nice to have someone to discuss ideas with when I
wanted to, I thank Colin de la Higuera for that.

Finally I would like to thank Fujinuma-san for being so nice with me
during the internship and for showing me so many thing, from Japanese
culture to technically interesting things.
...
The first one, named $S_{occ}$, is a log of the basic count of
occurrences of a precise lexical enhancement in our dictionary. We
define it in precise terms as:

\begin{equation*}
  S_{occ}(w \maps{D} r) = \log \proba{w \maps{D} r}
\end{equation*}
...
The score $S_{LM}$ captures the above definition, with $\proba[lm]{w}$ the
language model probability of $w$:

\begin{equation*}
  S_{LM}(w \maps{D} r) = \log \frac{\proba{w \maps{D} r \given
      w}}{\proba[lm]{w}}
\end{equation*}
...
\mathbb{1}, \mathbb{0}, \otimes, \oplus)$ (that we will use to combine
...
S$ the initial weight function; and $ρ : F \rightarrow \mathbb{K}$ the
final weight function mapping $F$ to $\mathbb{K}$.
...
$(\mathbb{K}, \mathbb{1}, \mathbb{0}, \otimes, \oplus)$ attached:
